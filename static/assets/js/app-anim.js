(function ($) {
    $(document).ready(function () {
        ...
        if (isPC()) {
            ...
            $('.qr-img[data-toggle="tooltip"]').tooltip({
                trigger: 'hover'
            });
        }
        // 初始化tab滑块
    ...
    $(document).on('click', "a[target!='_blank']", function () {
        if (theme.loading == '1' && $(this).attr('href') && $(this).attr('href').indexOf("#") != 0 && $(this).attr('href').indexOf("java") != 0 && !$(this).data('fancybox') && !$(this).data('commentid') && !$(this).hasClass('nofx')) {
            ...
            load.animate({
                opacity: '1'
            }, 200, 'swing').delay(2000).hide(300, function () {
                load.remove()
            });
        }
    });
            ...
            $.ajax({
                ...
                .done(function (response) {
                    if (response.trim()) {
                        var url = $(response);
                        box.html(url);
                        if (isPC()) url.find('[data-toggle="tooltip"]').tooltip({
                            trigger: 'hover'
                        });
                    } else {}
                    t.removeClass('disabled');
            ...
            $.ajax({
                ...
                .done(function (response) {
                    if (response.trim()) {
                        ...
                        if (isPC()) $('.ajax-url [data-toggle="tooltip"]').tooltip({
                            trigger: 'hover'
                        });
                    } else {
                        $('.ajax-loading').remove();
    ...
    function addSite(site, isLive, isHeader) {
        ...
        if (isPC()) $('.new-site[data-toggle="tooltip"]').tooltip({
            trigger: 'hover'
        });
    }
    ...
    function getUrlInfo(_url, modal) {
        ...
        $.post("//apiv2.iotheme.cn/webinfo/get.php", {
            url: _url,
            key: theme.apikey
...
function showAlert(data) {
    ...
    $html.show(200).delay(3500).hide(300, function () {
        $(this).remove()
    });
}
